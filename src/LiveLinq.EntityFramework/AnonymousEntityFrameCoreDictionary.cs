...
namespace LiveLinq.EntityFramework
{
    public class AnonymousEntityFrameCoreDictionary<TId, TDbDto, TDbContext> : EntityFrameworkCoreDictionaryBase<TId, TDbDto, TDbContext> where TDbDto : class where TDbContext : DbContext
    {
        ...
        public AnonymousEntityFrameCoreDictionary(Func<TDbContext, DbSet<TDbDto>> getDbSet, Func<TDbContext> createDbContext, Func<TDbDto, TId> getId, Func<DbSet<TDbDto>, TId, TDbDto> find = null, Action<DatabaseFacade> migrate = null) : base(migrate != null)
        {
            _find = find;
            if (_find == null)
            {
